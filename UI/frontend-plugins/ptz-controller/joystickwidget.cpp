...
void JoystickWidget::emitChanges()
{
	...
	if (m_state.zoom != m_prevState.zoom) {
		if (m_state.zoom > 0) {
			...
			if (DEBUG_ZOOM)
				qDebug()
					<< "Zoomed In at speed" << m_state.zoom;
		} else if (m_state.zoom < 0) {
			emit zoomedOut(m_state.zoom * -1);
			if (DEBUG_ZOOM)
				qDebug() << "Zoomed Out at speed"
					 << m_state.zoom * -1;
		} else {
			emit stoppedZooming();
			if (DEBUG_ZOOM)
